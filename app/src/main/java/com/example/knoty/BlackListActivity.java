...
public class BlackListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter.addItem(R.drawable.blacklist, "블랙 리스트", KnotyPreferences.getBoolean(this, KnotyPreferences.TOGGLE_BLACKLIST) ? 1 : 0, RecyclerAdapter.VIEW_TYPE_TOGGLE, null, new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked) {
                    ...
                    Toast.makeText(BlackListActivity.this, "화이트 리스트와 블랙 리스트 중 하나만 활성화 시킬 수 있습니다. 화이트 리스트를 끕니다.", Toast.LENGTH_LONG).show();
                } else {
        ...
        ArrayList<String> list = KnotyPreferences.getStringSet(this, KnotyPreferences.BLACKLIST_PREFERENCE);
        for(String str : list) {
                ...
                dialog.builder.setOnDismissListener(new DialogInterface.OnDismissListener() {
                    ...
                    public void onDismiss(DialogInterface dialog) {
                        if(flag) { //목록 추가 다이얼로그가 성공적으로 종료되면 Item을 추가시키고 새로고침
                            ArrayList<String> newList = KnotyPreferences.getStringSet(BlackListActivity.this,  KnotyPreferences.BLACKLIST_PREFERENCE);
                            adapter.addItem(-1, newList.get(newList.size() - 1), R.drawable.x_button, RecyclerAdapter.VIEW_TYPE_NORMAL, KnotyPreferences.BLACKLIST_PREFERENCE);
